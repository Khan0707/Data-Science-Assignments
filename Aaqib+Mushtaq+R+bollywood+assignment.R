...
# Store the movie name in variable named top_2_movie
	n<- length(bollywood$Movie)
	secnd<-sort(bollywood$Tcollection,partial=n-1)[n-1]
	top_2_movie <- bollywood[which(bollywood$Tcollection==secnd),1]
	  
	sorted<-bollywood[order(bollywood$Tcollection,decreasing = T),]
	top_2_movie<- sorted$Movie[2]
...
#You can write a command to find the maximum value of Ocollection, Wcollection, Fwcollecion and Tcollection using sapply
	mcollection<-sapply(bollywood[,4:7],FUN='max',na.rm=T)
